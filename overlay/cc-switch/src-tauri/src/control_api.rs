...
impl ControlConfig {
    fn validate(&self) -> Result<(), String> {
        ...
        if self.allowed_apps.is_empty()
            || self.allowed_apps.iter().any(|app| {
                AppType::from_str(app)
                    .map(|a| !a.supports_local_proxy())
                    .unwrap_or(true)
            })
        {
            return Err("control API requires an explicit supported application allowlist".into());
        }
        Ok(())
    }
}
...
async fn catalog(state: &AppState, app: AppType) -> Result<Catalog, ApiError> {
    ...
    Ok(Catalog {
        ...
        proxy_running: state.proxy_service.is_running().await && proxy.enabled,
        auto_failover: proxy.auto_failover_enabled,
    })
}
...
async fn select(
    ...
    let state = control.handle.state::<AppState>();
    let updated = apply_selection(state.inner(), app_type, &selection.id).await?;
...
async fn apply_selection(state: &AppState, app: AppType, id: &str) -> Result<Catalog, ApiError> {
...
async fn model_selection(
    ...
    let state = control.handle.state::<AppState>();
    let updated =
        apply_model_selection(state.inner(), app_type, &selection.id, &selection.model).await?;
    let _ = control.handle.emit(
        "provider-switched",
        json!({
            "appType": app,
...
async fn apply_model_selection(
    ...
    let mut provider = state
        ...
        .map_err(|_| error(StatusCode::INTERNAL_SERVER_ERROR, "read_failed"))?
        .ok_or_else(|| error(StatusCode::NOT_FOUND, "provider_not_found"))?;
    if !summarize(&provider, &app)
        .models
        .iter()
        .any(|candidate| candidate == model)
    {
        return Err(error(StatusCode::NOT_FOUND, "model_not_found"));
    }
    if !before.proxy_running {
        return Err(error(StatusCode::CONFLICT, "proxy_not_running"));
    }
        ...
        .and_then(|value| value.as_object_mut())
    {
        ...
        provider.settings_config["env"] = json!({ "ANTHROPIC_MODEL": model });
    }
    state
        .db
...
mod tests {
    ...
    async fn selection_updates_desktop_and_preserves_previous_on_failure() {
        ...
        assert!(apply_selection(&state, AppType::Claude, "second")
            .await
            .is_err());
        state
            .proxy_service
            ...
            .await
            .unwrap();
        let result = apply_selection(&state, AppType::Claude, "second")
            .await
            .unwrap();
        ...
        assert!(apply_selection(&state, AppType::Claude, "deleted")
            .await
            .is_err());
        assert_eq!(
    ...
    async fn codex_catalog_prefers_desktop_selection_and_switch_keeps_history() {
        ...
        assert!(matches!(
            apply_selection(&state, AppType::Codex, "db-marked").await,
        ...
        state
            ...
            .await
            .unwrap();
        let selected = apply_selection(&state, AppType::Codex, "db-marked")
            .await
            .unwrap();
    ...
    async fn model_selection_updates_current_provider_and_restore_backup() {
        ...
        let invalid = apply_model_selection(&state, AppType::Claude, "first", "missing").await;
        ...
        state
            ...
            .await
            .unwrap();
        let result = apply_model_selection(&state, AppType::Claude, "first", "model-b")
            .await
            .unwrap();
        assert_eq!(result.current_id, "first");
        assert_eq!(result.providers[0].model, "model-b");
        let provider = db.get_provider_by_id("first", "claude").unwrap().unwrap();
        assert_eq!(
            provider.settings_config.pointer("/env/ANTHROPIC_MODEL"),
            Some(&json!("model-b"))
        );
